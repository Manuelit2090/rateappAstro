...
var POST = async ({ request, cookies }) => {
	try {
		...
		const { business_id, rating, content, reviewItem } = jsonBody;
		const businessId = Number(business_id);
		const numericRating = Number(rating);
		if (!Number.isInteger(businessId) || businessId <= 0 || !Number.isFinite(numericRating) || numericRating < 1 || numericRating > 5) return jsonError("Datos inválidos", 400);
		...
		const [restaurants] = await pool.execute("SELECT id, slug, reviews FROM restaurants WHERE id = ?", [businessId]);
