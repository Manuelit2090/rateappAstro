...
import { I as createAstro, T as renderTemplate, b as renderComponent } from "./sequence_BMjlvBv6.mjs";
...
//#region src/components/UI/submitImages.vue
var _sfc_main$1 = {
	__name: "submitImages",
	props: {
		restaurantId: {
	...
	setup(__props, { expose: __expose }) {
		...
		const uploadImage = async () => {
			...
			try {
				...
				if (!presignedRes.ok) throw new Error(presignedData.error || "No se pudo obtener la URL firmada");
				...
				if (!uploadResponse.ok) throw new Error(`Error subiendo la imagen a R2 (${uploadResponse.status})`);
				if (!(await fetch("/api/restaurant", {
					method: "PUT",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
				...
				})).ok) throw new Error("Error guardando la imagen en la base de datos");
...
function _sfc_ssrRender$1(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
	_push(`<div${ssrRenderAttrs(mergeProps({ class: "flex flex-col gap-2 items-start" }, _attrs))}><p class="text-2xl font-bold">Sube la imagen de tu restaurante</p>`);
	if ($setup.currentImage) _push(`<div><img${ssrRenderAttr("src", $setup.currentImage)} alt="Imagen actual" class="preview max-w-xs rounded-lg shadow-md mb-2"></div>`);
	else _push(`<!---->`);
	_push(`<input class="file-input file-input-warning" type="file" accept="image/png, image/jpeg, image/webp"><button${ssrIncludeBooleanAttr(!$setup.selectedFile || $setup.isUploading || !$setup.props.restaurantId) ? " disabled" : ""} class="btn btn-primary mt-2">${ssrInterpolate($setup.isUploading ? "Subiendo..." : $setup.currentImage ? "Actualizar Imagen" : "Subir Imagen")}</button>`);
	if ($setup.imageUrl) _push(`<div class="result mt-4 flex flex-col gap-2"><p class="text-green-500 font-semibold">¡Subido con éxito!</p><input type="text"${ssrRenderAttr("value", $setup.imageUrl)} readonly class="input input-bordered w-full max-w-xs text-xs"></div>`);
	else _push(`<!---->`);
	_push(`</div>`);
}
...
_sfc_main$1.setup = (props, ctx) => {
	...
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("src/components/UI/submitImages.vue");
	return _sfc_setup$1 ? _sfc_setup$1(props, ctx) : void 0;
};
var submitImages_default = /*#__PURE__*/ _plugin_vue_export_helper_default(_sfc_main$1, [["ssrRender", _sfc_ssrRender$1]]);
//#endregion
	...
	setup(__props, { expose: __expose }) {
		...
		*/
		...
		const __returned__ = {
			...
			reviewAverage,
			saveRestaurant,
			SubmitImages: submitImages_default
		};
		Object.defineProperty(__returned__, "__isScriptSetup", {
			enumerable: false,
			value: true
...
var $$Settings = createComponent(async ($$result, $$props, $$slots) => {
	...
	if (restaurantId) try {
		const rResp = await fetch(new URL(`/api/restaurant?id=${restaurantId}`, Astro.url.origin).toString(), { headers: { Accept: "application/json" } });
		if (rResp.ok) (await rResp.json()).restaurant?.image;
	} catch (err) {
	...
	return renderTemplate`${renderComponent($$result, "AdminBaseLayout", $$AdminBaseLayout, { "title": "Ajustes del restaurante — Admin" }, { "default": ($$result) => renderTemplate`${renderComponent($$result, "RestaurantDashboard", RestaurantDashboard_default, {
		...
		"client:component-path": "/home/manuel/RateAppProject/rateappAstro/src/components/RestaurantDashboard.vue",
		"client:component-export": "default"
	})}` })}`;
}, "/home/manuel/RateAppProject/rateappAstro/src/pages/admin/settings.astro", void 0);
var $$file = "/home/manuel/RateAppProject/rateappAstro/src/pages/admin/settings.astro";
