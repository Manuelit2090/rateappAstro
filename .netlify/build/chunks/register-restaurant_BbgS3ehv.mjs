...
import { n as generateToken, r as hashPassword, t as buildAuthCookie } from "./auth_D5Os3erJ.mjs";
import { t as pool } from "./db_8ik5yfcT.mjs";
import { n as resolveCoordinates } from "./geocoding_D4Lcvtk1.mjs";
...
var POST = async ({ request }) => {
	try {
		...
		if (!coordinates) return new Response(JSON.stringify({ error: "No se pudo convertir la dirección en coordenadas válidas." }), {
			status: 400,
			headers: { "Content-Type": "application/json" }
		});
		...
		try {
			...
			const [restaurantInsert] = await connection.execute(`INSERT INTO restaurants (name, slug, address, category, latitude, longitude)
         VALUES (?, ?, ?, ?, ?, ?)`, [
				restaurantName,
				restaurantSlug,
				address,
