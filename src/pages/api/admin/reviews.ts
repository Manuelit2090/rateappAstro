/**
 * @file reviews.ts
 * @description Devuelve las reseñas y métricas del restaurante asociado a la sesión.
...
const RESPONSE_COLUMNS = ['restaurantResponse', 'ownerResponse', 'response', 'reviewResponse'];
 ...
 */
async function getResponseColumn(): Promise<string | null> {
  const [columns] = await pool.execute('SHOW COLUMNS FROM reviews') as any[];
...
export const GET: APIRoute = async ({ cookies }) => {
  ...
  if (!Number.isInteger(restaurantId) || restaurantId <= 0) {
    return jsonError('La sesión no tiene un restaurante asociado', 404);
  }

  try {
    const responseColumn = await getResponseColumn();
    const responseSelect = responseColumn
      ? `, r.${responseColumn} AS restaurantResponse`
      : ', NULL AS restaurantResponse';
    const [rows] = await pool.execute(
      `SELECT r.reviewId, r.reviewStar, r.reviewText, r.reviewUser, r.reviewDate${responseSelect}
       FROM reviews r
