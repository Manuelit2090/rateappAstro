...
import { Home, BarChart, Ticket, TicketX, Settings, MoreHorizontal, X, LogOut } from 'lucide-vue-next'
    ...
    <div
      ...
      <nav class="w-full max-w-sm flex items-center justify-around gap-1
        ...
        <a v-for="(it, idx) in items" :key="it.label + idx" :href="it.to" :class="[
          'flex flex-col items-center justify-center gap-0.5 flex-1 rounded-2xl py-2 transition-all duration-200 ellipsis',
          isActive(it.to, idx)
            ? 'bg-primary/10'
            : 'hover:bg-base-200'
        ]">
